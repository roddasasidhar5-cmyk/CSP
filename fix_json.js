...
// Since JSON.parse keeps the last occurrence of duplicate keys, we need to manually reconstruct
// Let's parse character by character to detect duplicates
// Actually, JSON.parse in V8 (Node.js) actually keeps the LAST value for duplicate keys
// So the data object already has the last values from the duplicate sections

// Let's verify what's in the current file
...
// Now we need to ensure the file is written with the correct merged data
// The issue is that JSON.parse keeps the LAST occurrence, but we need to make sure
// all the new keys from the second occurrence are preserved

// Let's manually build the correct English file
const correctData = {
  "common": data.common,
