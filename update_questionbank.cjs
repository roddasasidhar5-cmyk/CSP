...
const newQuestionBank = `// ==================== COMPREHENSIVE QUESTION BANK ====================
const questionBank = {
  mathematics: [
...
const startMarker = '// ==================== COMPREHENSIVE QUESTION BANK ====================';
const endMarker = '// ==================== SESSION MANAGER ====================';

const startIdx = content.indexOf(startMarker);
const endIdx = content.indexOf(endMarker);

if (startIdx === -1 || endIdx === -1) {
  console.error('ERROR: Could not find markers in script.js');
  console.error('startIdx:', startIdx, 'endIdx:', endIdx);
  process.exit(1);
}

const newContent = content.slice(0, startIdx) + newQuestionBank + '\n' + content.slice(endIdx + endMarker.length);

fs.writeFileSync('script.js', newContent, 'utf8');
console.log('script.js updated successfully with complete questionBank!');
